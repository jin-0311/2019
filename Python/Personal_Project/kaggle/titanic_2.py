...
data2 =pd.get_dummies(data1[['cabinlist','embarked','pclass','sex','title','alone','ti','age','fare','survived',
                             'age_range','family','fare_range']],
                      columns=['cabinlist','embarked','pclass','sex','title','alone','ti','age_range','family',
                               'fare_range'],
                      drop_first=True)
data2.head()
data2.info()
...
# 0.74641
forest=RandomForestClassifier(random_state=3, n_jobs=-1, criterion='gini', n_estimators=500, max_features='auto',
                              max_depth=5)
...
#모델 기반 특성 선택
feat_labels=train.drop('survived',axis=1).columns[0:]
importance=forest.feature_importances_
indices=np.argsort(importance)[::-1]
for f in range(x_train.shape[1]):
    print(f,feat_labels[indices[f]], importance[indices[f]])

# >>> title, sex, pclass, fare, cabinlist 까지 선택


from sklearn.feature_selection import SelectFromModel
from sklearn.ensemble import RandomForestClassifier
select =SelectFromModel(forest,threshold=0.1, prefit=True)
X_selected=select.transform(x_train)
print(X_selected.shape[1])
for f in range(X_selected.shape[1]):
    print(f+1,feat_labels[indices[f]], importance[indices[f]])
...
from sklearn.model_selection import train_test_split
x_train, x_valid, y_train, y_valid= train_test_split(X_train_scaled, Y_TRAIN,stratify=Y_TRAIN, random_state=1)
...
grid.best_params_  # 0.01
grid.score(X_train_scaled, Y_TRAIN)
y_pred=grid.predict(X_test_scaled)
y_pred


# SVC 0.78468 (C=1, gamma=0.01)
...
grid= SVC(random_state=1, C=1, gamma=0.01)
...
X_train_pca = kpca.fit_transform(X_train_scaled)
X_test_pca = kpca.transform(X_test_scaled)
...
param_range = [0.0001, 0.001, 0.01, 0.1, 1, 1.0, 10.0, 100.0]
param_grid = [{'svc__C': param_range, 'svc__kernel': ['linear']},
              {'svc__C': param_range, 'svc__gamma': param_range, 'svc__kernel': ['rbf']}]
grid = GridSearchCV(estimator=pipe_svc, param_grid=param_grid, scoring='accuracy', cv=10, n_jobs=-1)
grid.fit(X_train_scaled, Y_TRAIN)
grid.best_params_
...
grid = GridSearchCV(GradientBoostingClassifier(random_state=0), param_grid, cv=5,
...
from sklearn.model_selection import GridSearchCV
grid= MLPClassifier(random_state=3, max_iter=1000, hidden_layer_sizes=[64,64,64], validation_fraction=0.5,
...
mlp=MLPClassifier(random_state=3, max_iter=1000, activation='relu',hidden_layer_sizes=(10,10),batch_size=64)
...
X_train=X_train_scaled.reshape(891,40).astype('float32')
X_test=X_test_scaled.reshape(418,40).astype('float32')
...
model=Sequential()
model.add(Dense(128,input_shape=(40,), activation='tanh'))
